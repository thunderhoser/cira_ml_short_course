...
def _get_histogram(input_values, num_bins, min_value, max_value):
    ...
    inputs_to_bins = numpy.digitize(input_values, bin_cutoffs, right=False) - 1
...
def _plot_forecast_histogram(figure_object, num_examples_by_bin):
    ...
    y_max = _floor_to_nearest(
        1.05 * numpy.max(bin_frequencies), HISTOGRAM_Y_TICK_SPACING
    )
    num_y_values = 1 + int(numpy.round(
        y_max / HISTOGRAM_Y_TICK_SPACING
    ))

    y_tick_values = numpy.linspace(0, y_max, num=num_y_values)
